...
export async function handleFUKondisi(row, { db, sendFonnteMessage }) {
  ...
  if (tanggalSekarang !== tanggalJadwal) {
    prettyLog.info({ id: row.id, nama: row.nama, tanggalJadwal }, `⏳ FU Kondisi belum waktunya dikirim`);
    return;
  }
  ...
  if (res?.status) {
    ...
    await db.query(`
      ...
      VALUES (?, ?, ?, 'sukses', 'Pesan FU Kondisi berhasil dikirim')
    `, [row.id, row.no_hp, row.wa_status]);
    prettyLog.info({ id: row.id, nomor: row.no_hp }, `✅ FU Kondisi berhasil dikirim`);
  } else {
    await db.query(`UPDATE wa_gateway SET status_send='gagal' WHERE id=?`, [row.id]);
    prettyLog.error({ id: row.id, nomor: row.no_hp }, `❌ Gagal kirim FU Kondisi`);
  }
}
