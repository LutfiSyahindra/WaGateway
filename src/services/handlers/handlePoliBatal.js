...
export async function handlePoliBatal(row, { db, sendFonnteMessage }) { 
    ...
    const pesan = renderTemplate(templates.poli_batal, formattedData);
    ...
    await db.query(`
        ...
        VALUES (?, ?, ?, 'sukses', 'Pesan Poli Batal berhasil dikirim')
    `, [row.id, row.no_hp, row.wa_status]);
    prettyLog.info({ id: row.id, nomor: row.no_hp }, `✅ Poli Batal berhasil dikirim`);
    } else {
        await db.query(`UPDATE wa_gateway SET status_send='gagal' WHERE id=?`, [row.id]);
        prettyLog.error({ id: row.id, nomor: row.no_hp }, `❌ Gagal kirim Poli Batal`);
    }
}